...
(function enhanceImageLoading() {
  ...
  function useCDN(img) {
    ...
    if (src && src.indexOf(window.location.host) !== -1 && src.indexOf('cdn.jsdelivr.net') === -1) {
      ...
      if (lazySrc && lazySrc.indexOf(window.location.host) !== -1) {
        try {
          var url3 = new URL(lazySrc);
  ...
  document.addEventListener('DOMContentLoaded', function() {
    // 给所有图片添加原生 lazy loading 和异步解码
    var images = document.querySelectorAll('img');
    images.forEach(function(img) {
      var src = img.src || img.getAttribute('data-src') || img.getAttribute('data-lazy-src') || '';
      if (src && !src.includes('data:') && !src.includes('favicon') && !src.includes('.svg')) {
        img.loading = 'lazy';
        img.decoding = 'async';
      }
    });
    ...
    document.querySelectorAll('img').forEach(function(img) {
      var src = img.src || img.getAttribute('data-src') || img.getAttribute('data-lazy-src') || '';
      if (src && src.indexOf('cdn.jsdelivr.net') === -1 && src.indexOf(window.location.host) !== -1) {
        useCDN(img);
      }
    });
        ...
        mutations.forEach(function(m) {
          m.addedNodes.forEach(function(node) {
            ...
            if (node.querySelectorAll) {
              node.querySelectorAll('img').forEach(function(subImg) {
                subImg.loading = 'lazy';
                subImg.decoding = 'async';
                useCDN(subImg);
              });
            }
          });
...
function createFloatHomeButton() {
  ...
  function positionHomeButtonBelowBackToTop(homeBtn) {
    ...
    // 查找"回到顶部"按钮
    var backToTopBtn = rightside.querySelector('#go-up') ||
                       rightside.querySelector('[onclick*="scrollTo"]') ||
                       rightside.querySelector('.fa-arrow-up') ||
                       rightside.querySelector('a[href="#"]:last-of-type');

    if (!backToTopBtn) {
      // 查找 rightside 的最后一个子元素，在它之后插入
      var lastChild = rightside.lastElementChild;
      while (lastChild && lastChild.lastElementChild) {
        lastChild = lastChild.lastElementChild;
      }
      if (lastChild && lastChild.parentElement) {
        lastChild.parentElement.appendChild(homeBtn);
      } else {
        rightside.appendChild(homeBtn);
      }
    } else {
      // 查找"回到顶部"按钮的父元素
      var parent = backToTopBtn.parentElement || backToTopBtn.closest('a,div,button');
      if (parent && parent.parentElement === rightside) {
        // 在"回到顶部"按钮后面插入
        parent.insertAdjacentElement('afterend', homeBtn);
      } else if (backToTopBtn.parentElement) {
        backToTopBtn.parentElement.appendChild(homeBtn);
      } else {
        rightside.appendChild(homeBtn);
      }
    }
  ...
  homeBtn.addEventListener('mouseenter', function() {
    this.style.backgroundColor = 'var(--mario-yellow)';
    this.style.color = 'var(--mario-red)';
